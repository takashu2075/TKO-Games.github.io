...
/**
 * Quadtree broadphase implementation
 * Reference: http://marupeke296.com/COL_2D_No8_QuadTree.html
 */
class QuadtreeBroadphase extends Broadphase {
  ...
  collisionPairs(world, pairs1, pairs2) {
    for (const body of world.bodies) {
      if ((body.type & Body.STATIC) !== 0 || body.sleepState === Body.SLEEPING) body.updateAABB();
    }

    const quadtree = world.quadtree;
    const nodes = quadtree.nodes;
    const bodyStack = [];
    this.inspectNode(nodes, 0, bodyStack, pairs1, pairs2);
  }

  inspectNode(nodes, nodeIndex, bodyStack, pairs1, pairs2) {
    // 現在の空間内に登録されているオブジェクト同士を衝突
    let obj1 = nodes[nodeIndex].firstObject;
    const tmpStack = [];

    while (obj1) {
      const bi = obj1.body;
      tmpStack.push(bi);
      let obj2 = obj1.nextObject;
      ...
      while (obj2) {
        ...
        obj2 = obj2.nextObject;
      } // スタック（リスト）に登録されているオブジェクトとの衝突判定


      for (const stack of bodyStack) {
        if (this.needBroadphaseCollision(bi, stack)) {
          this.intersectionTest(bi, stack, pairs1, pairs2);
        }
      } // 次のオブジェクトへ移行


      obj1 = obj1.nextObject;
    } // スタックに現在の空間内のオブジェクトを追加


    for (const stack of tmpStack) {
      bodyStack.push(stack);
    } // 小空間があるかチェック


    for (let i = 1; i <= 4; i++) {
      const childNodeIndex = (nodeIndex << 2) + i;

      if (nodes[childNodeIndex]) {
        // 小空間があったので移動
        this.inspectNode(nodes, childNodeIndex, bodyStack, pairs1, pairs2);
      }
    } // 戻り処理
    // スタックからオブジェクトを外す
...
class Heightfield extends Shape {
  ...
  calculateWorldAABB(pos, quat, min, max) {
    /** @TODO do it properly */
    const size = this.elementSize * (this.data.length - 1);
    const offset = 0.1; // min.set(pos.x + offset, -Number.MAX_VALUE, pos.z + offset);
    // max.set(pos.x + size - offset, Number.MAX_VALUE, pos.z + size - offset);

    min.set(pos.x - size + size + offset, -Number.MAX_VALUE, pos.z - size + offset);
    max.set(pos.x + size - offset, Number.MAX_VALUE, pos.z - offset);
  }
...
class QuadtreeNode {
  ...
  insert(object) {
    const oldFirstObject = this.firstObject;
    object.nextObject = oldFirstObject;
    object.node = this;
    if (oldFirstObject) oldFirstObject.prevObject = object;
    this.firstObject = object;
    this.numObject++;
  }

  remove(object) {
    if (object.prevObject) object.prevObject.nextObject = object.nextObject;
    if (object.nextObject) object.nextObject.prevObject = object.prevObject;
    if (this.firstObject == object) this.firstObject = object.nextObject;
    object.prevObject = undefined;
...
class Quadtree {
  constructor(size, maxDepth) {
    this.size = size ? size : 131072;
    this.offset = this.size / 2;
    this.maxDepth = maxDepth ? maxDepth : 3;
    this.nodes = [];
    const offset = (Math.pow(4, this.maxDepth) - 1) / 3;
    const number = Math.pow(Math.pow(2, this.maxDepth), 2);

    for (let i = 0; i < offset + number; i++) {
      this.nodes.push(new QuadtreeNode());
    }

    this.minNodeSize = this.size / Math.pow(2, this.maxDepth);
  }

  addBody(body) {
    const aabb = body.aabb;
    const index = this.getNodeIndex(aabb.lowerBound.x + this.offset, aabb.lowerBound.z + this.offset, aabb.upperBound.x + this.offset, aabb.upperBound.z + this.offset);
    const node = this.nodes[index] ? this.nodes[index] : this.nodes[0];
    const object = new QuadtreeObject(body, node);
    node.insert(object);
  }

  update() {
    for (const node of this.nodes) {
      let object = node.firstObject;

      while (object) {
        const nextObject = object.nextObject;
        const aabb = object.body.aabb;
        const index = this.getNodeIndex(aabb.upperBound.x + this.offset, aabb.upperBound.z + this.offset, aabb.lowerBound.x + this.offset, aabb.lowerBound.z + this.offset);
        const properNode = this.nodes[index] ? this.nodes[index] : this.nodes[0];

        if (node !== properNode) {
          node.remove(object);
          properNode.insert(object);
        }
  ...
  removeBody(body) {
    const object = body.quadtreeObject;
    const node = object == null ? void 0 : object.node;
  ...
  getNodeIndex(minX, minY, maxX, maxY) {
    const minIndex = this.getIndex(minX, minY);
    const maxIndex = this.getIndex(maxX, maxY);
    const tmpNumber = minIndex ^ maxIndex;
    const depth = this.getDepth(tmpNumber);
    const mortonIndex = this.getMortonIndex(maxIndex, depth);
    return this.getSerializedIndex(depth, mortonIndex);
  }

  getIndex(x, y) {
    const nx = Math.floor(x / this.minNodeSize);
    const ny = Math.floor(y / this.minNodeSize);
    return this.bitSeparate32(nx) | this.bitSeparate32(ny) << 1;
  }
  ...
  getDepth(n) {
    ...
    return this.maxDepth - found;
  }

  getMortonIndex(n, depth) {
    return n >> 2 * (this.maxDepth - depth);
  }

  getSerializedIndex(depth, mortonIndex) {
    return (Math.pow(4, depth) - 1) / 3 + mortonIndex;
  }
...
class World extends EventTarget {
  ...
  constructor(options) {
    ...
    this.broadphase = options.broadphase !== undefined ? options.broadphase : new QuadtreeBroadphase();
    this.bodies = [];
    ...
    this.quadtree = new Quadtree(options.quadtreeSize, options.quadtreeDepth);
  }
  /**
  ...
  internalStep(dt) {
    const internalStepStart = performance.now();
    this.dt = dt;
    ...
    this.quadtree.update(); // Collision detection

    if (doProfiling) {
      profile.quadtreeUpdate = performance.now() - profilingStart;
      profilingStart = performance.now();
    ...
    if (doProfiling) {
      profile.afterQuadtreeUpdate = performance.now() - profilingStart;
      profilingStart = performance.now();
    } // Collision detection
    ...
    if (doProfiling) {
      profile.afterBroadphase = performance.now() - profilingStart;
      profilingStart = performance.now();
    } // Generate contacts
    ...
    if (doProfiling) {
      profile.afterSolve = performance.now() - profilingStart;
      profilingStart = performance.now();
    }
    ...
    if (doProfiling) {
      profile.afterIntegrate = performance.now() - profilingStart;
    }

    profile.internalStep = performance.now() - internalStepStart;
  }
